...
export default function StudentApplyForm({user, profile, setShowApplyForm, reload}) {
    ...
    function validate(){
        ...
        if (!form.preferredStartDate) return "Please select a start date.";
        if (!form.preferredEndDate) return "Please select an end date.";
        if (new Date(form.preferredEndDate) < new Date(form.preferredStartDate)) return "End date cannot be before start date.";
        ...
        async function handleSubmit(e) {
            ...
            try {
                ...
                if (coverLetterFile) {
                    ...
                    const res = await fetch(CLOOUDINARY_UPLOAD_URL, {
                        method: "POST",
                        body: formData,
                    });
                    if (!res.ok) {
